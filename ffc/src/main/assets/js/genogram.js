...
function genogramSymbol(data, spouse,twin,idCard) {
    ...
    this.GenerateGenogram = function (id) {
        ...
        if(result.length>0) {
            ...
            if (dataFather != null || this.dataMother!=null) {
                ...
                // this.CMale(dataFather[0].id, 200, 200);
                var util = new Utility();
                var objParent = document.getElementById(dataParent[0].id);
                var pos = util.GetTopLeftXY(objParent) 
    ...
    this.GetFamilyDetail = function (id, level, index) {
        ...
        if (main == null || main == undefined) {
            ...
            if (data.length > 0) {
                if (data[0].sex == "Male") {
                    this.CMale(id, 100 + (100 * index), (stepLevel * (level + 1)));
                }
                else if (data[0].sex == "Female") {
                    this.CFemale(id, 100 + (100 * index), (stepLevel * (level + 1)));
                }
                else if (data[0].sex == "Unknow") {
                    this.GenderUnknown(id, 100 + (100 * index), (stepLevel * (level + 1)));
                }
            }
        }
        var spouselst = this.FindSpouse(id, this.spouse);
...
function TransformData(sourceGenogram){
               ...
               // alert(JSON.stringify(data));
                ...
                if(data.id == id){
                    ...
                    if(data.relationships!=null){
                        ...
                        obj.age = this.getAge(obj.birthday,obj.deathdate);
    ...
    this.getAge = function(birthday,deathdate){
        var age="";
