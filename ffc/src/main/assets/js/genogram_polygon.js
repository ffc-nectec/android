...
function genogramSymbol(data, spouse,twin,idCard) {
        ...
        if (obj != null) {
            if (obj.length > 0) {
                ...
                if (lineObj != null) {
                    ...
                    x2Line = lineObj.x2.baseVal.value;
                    ...
                    var lstPolygon = [];
                    var util = new Utility();
                    var lstColor=['#000000','#DC143C','#008000','#4169E1','#8B008B','#FF6347','#FF1493'];
                    for (var i = 0; i < lstChild.length; i++) {
                        ...
                        if (pos.length > 0) {
                            ...
                            var objLine = document.createElementNS(svgns, 'polyline');
                            var points = "";
                            //points = x2+","+y2+" "+x2+","+y1;
                            if(x2<x1Line && x2 >x2Line){
                                points = x2+","+y2+" "+x2+","+y1;
                            }
                             else {
                                points = x0+","+(y1)+" "+x0+","+((y1+y2)/2) +" "+x2+","+((y1+y2)/2)+" "+x2+","+y2;
                                //points = x0+","+(y1)+" "+x0+","+(1*(y1+y2)/2+(lstChild.length*(i*4))) +" "+x2+","+(1*(y1+y2)/2+(lstChild.length*(i)*4))+" "+x2+","+y2;
                                // if(lstChild.length==1){
                                //     points = x0+","+(y1)+" "+x0+","+(0.90*(y1+y2)/2+(lstChild.length*(i*5))) +" "+x2+","+(0.90*(y1+y2)/2+(lstChild.length*(i*5)))+" "+x2+","+y2;
                                // }
                                // else {
                                //     points = x0+","+(y1)+" "+x0+","+(1*(y1+y2)/2+(lstChild.length*(i*4))) +" "+x2+","+(1*(y1+y2)/2+(lstChild.length*(i)*4))+" "+x2+","+y2;
                                // }
                            }
                            objLine.setAttributeNS(null, 'points',points);
                            // objLine.setAttributeNS(null, 'y1', y1);
                            // objLine.setAttributeNS(null, 'x2', x2);
                            // objLine.setAttributeNS(null, 'y2', y2);
                            objLine.setAttributeNS(null,'fill','none');
                            objLine.setAttributeNS(null, 'stroke', lstColor[i%lstColor.length]);
    ...
    this.GenerateGenogram = function (id) {
        ...
        if(result.length>0) {
            ...
            if (dataFather != null) {
                ...
                this.GetFamilyDetail(dataFather[0].id, level + 1,1);
    ...
    this.GetFamilyDetail = function (id, level, index) {
        ...
        if (main == null || main == undefined) {
            ...
            if (data.length > 0) {
                if (data[0].sex == "Male") {
                    this.CMale(id, 100 + (100 * index), (step * (level + 1)));
                }
                else if (data[0].sex == "Female") {
                    this.CFemale(id, 100 + (100 * index), (step * (level + 1)));
                }
                else if (data[0].sex == "Unknow") {
                    this.GenderUnknown(id, 100 + (100 * index), (step * (level + 1)));
                }
        ...
        if (spouselst.length > 0) {
            ...
            if (data.length > 0) {
                if (data[0].sex == "Male") {
                    this.CMale(id, 100 + (100 * index), 200 + (120 * level));
                }
                else if (data[0].sex == "Female") {
                    this.CFemale(id, 100 + (100 * index), 200 + (120 * level));
                }
            }
        }
    }
